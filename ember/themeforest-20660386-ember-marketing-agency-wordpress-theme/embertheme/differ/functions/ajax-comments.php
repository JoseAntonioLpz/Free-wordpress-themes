...
function differ_add_ajax_comment() {

	check_ajax_referer( 'differ_comments', 'nonce_code' );
	...
	if ( ! comments_open( $comment_post_ID ) ) {
		...
		wp_die( esc_attr( 'Sorry, comments are closed for this item.', 'embertheme' ) );
	} elseif ( 'trash' == $status ) {
	...
	if ( $user->exists() ) {
		...
		if ( get_option( 'comment_registration' ) || 'private' == $status ) {
			wp_die( esc_html__( 'You must be %1$slogged in%2$s to post a comment.', 'embertheme' ) );
		}
	}
	...
	if ( get_option( 'require_name_email' ) && ! $user->exists() ) {
		if ( 6 > strlen( $comment_author_email ) || '' == $comment_author ) {
			wp_die( esc_html__( 'Error: please fill all required fields', 'embertheme' ) );

		} elseif ( ! is_email( $comment_author_email ) ) {
			wp_die( esc_html__( 'Error: incorrect email', 'embertheme' ) );
		}
	}

	if ( '' == trim( $comment_content ) || '<p><br></p>' == $comment_content ) {
		wp_die( esc_html__( 'Error: please type comment ', 'embertheme' ) );
...
add_action( 'wp_ajax_differ_comments', 'differ_add_ajax_comment' );
add_action( 'wp_ajax_nopriv_differ_comments', 'differ_add_ajax_comment' );