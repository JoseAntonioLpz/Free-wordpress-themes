...
if (!function_exists('umbala_ajax_search')) {
    function umbala_ajax_search() {
        ...
        check_ajax_referer( 'umbala-ajax-search', 'security' );
...
/* Social Button */
if ( !function_exists('umbala_social_button') ) {
    function umbala_social_button() {
        if ( wp_is_mobile() && !umbala_get_opt('alus_social_button_mobile') || !wp_is_mobile() && umbala_get_opt('alus_social_button') ) {
        ?>

        <div class="umbala-social-button <?php echo 'social-'.umbala_get_opt('alus_social_position') ?>">
            <?php if( umbala_get_opt('alus_social_type') == 'share' && function_exists('alus_template_social_sharing') ): ?>
               <?php alus_template_social_sharing(); ?>

            <?php elseif( umbala_get_opt('alus_social_type') == 'follow' && function_exists('alus_template_social_network') ): ?>
                <?php alus_template_social_network(); ?>
            <?php endif; ?>
        </div>
    ...
    add_action('wp_footer', 'umbala_social_button');
}

/* Cookie Notice */
if ( !function_exists('umbala_cookie_notice') ) {
    function umbala_cookie_notice() {
        if ( !umbala_get_opt('alus_cookie_notice') ) {
            return;
        }
        $page_cookie = umbala_get_opt('alus_cookie_info_link');
        ?>

        <div class="umbala-cookie-notice">
            <div class="cookie-wrapper">
                <div class="cookie-text">
                    <?php echo umbala_get_opt('alus_cookie_text'); ?>
                </div>
                <div class="cookie-button">
                    <?php if ($page_cookie): ?>
                        <a href="<?php echo esc_url(get_permalink( $page_cookie )); ?>" class="cookie-info">
                            <?php esc_html_e('Read more', 'umbala'); ?>
                        </a>
                    <?php endif; ?>
                    <a href="#" class="cookie-accept"><?php esc_html_e('Accept', 'umbala'); ?></a>
                </div>
            </div>
        </div>
    ...
    add_action('wp_footer', 'umbala_cookie_notice');
}
...
/** Custom 404 Page * */
if ( !function_exists('umbala_custom_404_page') ) {
    function umbala_custom_404_page( $page_template ) {
        global $wp_query;
        $custom_404 = umbala_get_opt( 'alus_custom_404_page' );
        if ( umbala_get_opt( 'alus_enable_custom_404_page' ) && ! empty( $custom_404 )  ) {
    ...
    add_filter( '404_template', 'umbala_custom_404_page', 999 );
}

/** Maintenance Mode * */
if ( !function_exists('umbala_maintenance_mode') ) {
    function umbala_maintenance_mode() {
        $maintenance_page = umbala_get_opt( 'alus_maintenance_mode_page' );
        if( ! umbala_get_opt( 'alus_maintenance_mode' ) || is_user_logged_in() || empty( $maintenance_page ) ) {
            return;
        }
    ...
    add_action( 'template_redirect', 'umbala_maintenance_mode');
}
